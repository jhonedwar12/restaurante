...
<div className="flex flex-col sm:flex-row items-center sm:space-x-2 space-y-2 sm:space-y-0">
  ...
  <SheetHeader>
    <SheetTitle className="text-center text-gray-500 dark:text-amber-400 py-8">Carrito de Compras</SheetTitle>
  ...
  {!pedidosHabilitados ? (
    ...
    <>
      <div className="flex-1 overflow-y-auto mt-1 space-y-1">
        {cart.length === 0 ? (
          <p className="text-center text-gray-500 dark:text-gray-100 py-8">Tu carrito está vacío</p>
        ) : (
          cart.map((item) => (
            <CartItem
              key={item.id}
